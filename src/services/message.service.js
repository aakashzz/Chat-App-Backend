...
export async function getChatUserProfileService(userId,chatId){
   ...
   const responseOfGetUser = await Message.find({chatId:chatId}).populate("chatId");
   const filterGetUser = responseOfGetUser.map((value) => {
      return value.chatId.filter((data) => !data.equals(userId));
   });
   console.log(responseOfGetUser)
   if(!filterGetUser)throw new ApiError(400,"Not Filter User Details Please Check")
